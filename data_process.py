...
def process_data(pra_now_dict, pra_start_ind, pra_end_ind, pra_observed_last):
    ...
    for frame_ind in range(pra_start_ind, pra_end_ind):
        # we add mark "1" to the end of each row to indicate that this row exists, using list(pra_now_dict[
        # frame_ind][obj_id])+[1] -mean_xy is used to zero_centralize data now_frame_feature_dict = {obj_id : list(
        # pra_now_dict[frame_ind][obj_id]-mean_xy)+[1] for obj_id in pra_now_dict[frame_ind] if obj_id in
...
>>>>>>> 5c4647045af7f08bf65cebffa2e3f76d575e51d2
