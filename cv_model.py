...
if __name__ == '__main__':
    cross_data_list = np.array(pd.read_csv("data/frame0901-1013.txt", sep=' '), dtype=np.float64)
    all_data_list = np.array(pd.read_csv("data/frame0901-1013.txt", sep=' '), dtype=np.float64)
    dict_x = {}
    ...
    for row in cross_data_list:
        for j in range(6):
            ...
            if not str(list[predict_frame, int(row[object_id])]) in dict_x:
                continue
            print(0.5 * row[speed] * math.cos(math.pi / 2 - row[heading]) * (j + 1))
            print(math.sin(math.pi / 2))
            predict_x_position = row[position_x] + 0.5 * row[speed] * math.cos(math.pi / 2 - row[heading]) * (j + 1)
            predict_y_position = row[position_y] + 0.5 * row[speed] * math.sin(math.pi / 2 - row[heading]) * (j + 1)
